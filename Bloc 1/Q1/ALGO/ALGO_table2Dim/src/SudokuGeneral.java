...
public class SudokuGeneral {
	...
	public boolean estPresentDansLigne(int nombre, int indiceLigne) {
		...
		for ( int i=0; i<grille[indiceLigne].length; i++ ){
	...
	public boolean estPresentDansRegion(int nombre, int indiceLigneSupGauche, int indiceColonneSupGauche) {
		...
		int tailleR = (int) Math.sqrt(grille.length);

		for ( int i=indiceLigneSupGauche; i<indiceLigneSupGauche+tailleR; i++ ){

			for ( int j=indiceColonneSupGauche; j<indiceColonneSupGauche+tailleR; j++ ){
	...
	public int nombreLePlusPresent() {
		...
		for ( int i=0; i < grille.length; i++ ){
			...
			for ( int j=0; j < grille[i].length; j++ ){
				...
				int indiceNombre = grille[i][j];

				frequences[indiceNombre]++;
		...
		if ( frequences[0] == grille.length * grille.length ) return -1;
