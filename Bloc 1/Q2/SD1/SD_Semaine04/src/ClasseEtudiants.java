...
public class ClasseEtudiants{
	...
	public SerieEtudiants getSerieEtudiants(int numeroSerie)throws IllegalArgumentException{
		...
		SerieEtudiants serie = tableSeries[numeroSerie-1];
	...
	public int numeroSerie(String nom){
		...
		for ( SerieEtudiants serie : tableSeries ) {

			if ( serie.contientEtudiant( nom ) ) return serie.getNumeroSerie();
	...
	public boolean changerSerie(String nom, int nouveauNumeroSerie){
		...
		if ( nom == null || nom.equals( "" ) || nouveauNumeroSerie < 1 || nouveauNumeroSerie >= tableSeries.length ) throw new IllegalArgumentException();

		int numeroSerieActuel = numeroSerie( nom );

		// Déjà présent dans la série

		if ( numeroSerieActuel == nouveauNumeroSerie ) return false;
