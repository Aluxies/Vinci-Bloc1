...
public class DrapeauBelge {
	...
	public DrapeauBelge() {
		...
		NoeudCouleur dernierRouge = new NoeudCouleur( 'r' );

		premierNoir.suivant = dernierJaune;
		dernierJaune.suivant = dernierRouge;
	...
	public void ajouter(char couleur){
		...
		if ( couleur != 'r' && couleur != 'n' && couleur != 'j' ) throw new IllegalArgumentException();

		NoeudCouleur nouveauNoeud = new NoeudCouleur( couleur );

		if ( couleur == 'n' ){

			nouveauNoeud.suivant = premierNoir.suivant;
			premierNoir.suivant = nouveauNoeud;
