...
public class ListeRecCaracteres {
	...
	public ArrayList<Character> enArrayList(){
		...
		enArrayList( tete, arrayList );

		return arrayList;
		
	}

	private void enArrayList(NoeudCaractere noeud, ArrayList<Character> arrayList ){

		if ( noeud == null ) return;

		arrayList.add( noeud.caractere );

		enArrayList( noeud.suivant, arrayList );
	...
	private boolean estEgalA( NoeudCaractere noeud1, NoeudCaractere noeud2 ){
		...
		if ( noeud1 == null && noeud2 != null || noeud1 != null && noeud2 == null ) return false;
...
//	}
	...
	public ListeRecCaracteres clone(){
		...
		ListeRecCaracteres listeRecCaracteres = new ListeRecCaracteres();

		listeRecCaracteres.tete = clone( tete );

		return listeRecCaracteres;
		
	}

	private NoeudCaractere clone( NoeudCaractere noeud ){

		NoeudCaractere clone = null;

		if ( noeud == null ) return clone;

		NoeudCaractere new_noeud = new NoeudCaractere( noeud.caractere, clone( noeud.suivant ) );
	...
	public int supprimerToutesLesOccurrences(char caractereASupprimer){
		...
		NoeudCaractere bidon = new NoeudCaractere( ' ', tete );

		return supprimerToutesLesOccurrences( bidon, caractereASupprimer );
	
	}

	private int supprimerToutesLesOccurrences( NoeudCaractere noeud, char caractereASupprimer ){

		if ( noeud.suivant == null ) return 0;

		if ( noeud.suivant.caractere == caractereASupprimer ){

			noeud.suivant = noeud.suivant.suivant;

			return 1 + supprimerToutesLesOccurrences( noeud.suivant, caractereASupprimer );

		}

		return supprimerToutesLesOccurrences( noeud.suivant, caractereASupprimer );
