public class TestEnsembleVoituresAutorisees{
	...
	public static void main(String[] args) {	
		...
		if(e.retirerVoiture(new Voiture("1BBB000",""))){
			System.out.println("la voiture est toujours présente dans l'ensemble alors qu'elle ne devrait pas KO ");
			return;
		} else {
			System.out.println("la voiture n'est pas présente dans l'ensemble OK");
		}

		System.out.println("Cas 6 : nouvelle verification de la presence des voitures 1AAA000 et 1CCC000");

		if (e.voitureAutorisee(new Voiture("1AAA000",""))){
			System.out.println("la voiture 1AAA000 est présente dans l'ensemble OK");
		}else{
			System.out.println("la voiture 1AAA000 n'est pas présente dans l'ensemble KO");
			return;
		}
		if ( e.voitureAutorisee(new Voiture("1CCC000",""))){
			System.out.println("la voiture 1CCC000 est présente dans l'ensemble OK");
		}else{
			System.out.println("la voiture 1CCC000 n'est pas présente dans l'ensemble KO");
			return;
		}
		...
		if ( e.ajouterVoiture(new Voiture("1AAA000",""))){
			System.out.println("ajout de la voiture 1AAA000 a réussi alors qu'elle est déjà présente KO");
			return;
		}else{
			System.out.println("ajout de la voiture 1AAA000 a échoué car elle est déjà présente OK");
		}
