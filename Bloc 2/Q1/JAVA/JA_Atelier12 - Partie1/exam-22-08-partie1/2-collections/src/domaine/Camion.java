...
public class Camion  {
    ...
    private SortedMap<LocalDate,Trajet> trajets = new TreeMap<LocalDate, Trajet>();
    ...
    public boolean ajouterTrajet(Trajet trajet){
        ...
        Trajet trajetPresent = trajets.get( trajet.getDate() );

        if ( trajetPresent == null ) {

            LocalDate dateCourante = trajet.getDate();
            Trajet trajetPrecedent = trajets.get( dateCourante.minusDays( 1 ) );

            if ( trajetPrecedent != null ) {

                if (!trajetPrecedent.getVilleArrivee().equals( trajet.getVilleDepart() ) )
                    return false;

            }

            Trajet trajetSuivant = trajets.get( dateCourante.plusDays( 1 ) );

            if ( trajetSuivant != null ) {

                if ( !trajetSuivant.getVilleDepart().equals( trajet.getVilleArrivee() ) )
                    return false;
