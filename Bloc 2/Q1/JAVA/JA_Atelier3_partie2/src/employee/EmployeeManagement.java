...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        ...
        try {
            ...
            String path = "./resources/streamingvf.csv";
    ...
    private static String firstLine() {
        ...
        return supplier.get().toList().get(0);
    ...
    private static List<String> filteredLastnames() {
        ...
        Stream<Employee> streamEmployees = supplier.get().map( e -> new Employee(e) );

        Predicate<String> predicate1 = s -> s.length() > 8;
        Predicate<String> predicate2 = s -> s.contains("O") || s.contains("K");
        ...
        return streamEmployees.map( e -> e.getLastname() ).filter( predicate1.and( predicate2 ) ).toList();
    }
    ...
    private static List<Integer> occurencesOfE() {
        ...
        BiFunction<Employee,Character,Integer> biFunction = (e,c) ->
                (int) e.getFirstname().chars().filter(ch -> ch == c ).count();
    ...
    private static Map<Boolean, List<Integer>> timeDistrubution() {
        ...
        Map<Object, List<Integer>> employeesMap = new HashMap<Object, List<Integer>>();
